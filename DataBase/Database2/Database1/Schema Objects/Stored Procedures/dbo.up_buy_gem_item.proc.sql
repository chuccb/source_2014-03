CREATE PROCEDURE [dbo].[up_buy_gem_item]
	@strBuyerLogin_ [nvarchar](20),
	@strOwnerLogin_ [nvarchar](20),
	@iGoodsID_ [int],
	@iDuration_ [int]
AS
exec dbo.up_log_set_proc_count 'up_buy_gem_item'

SET NOCOUNT ON

DECLARE   @iGem                    int,   --구입에 필요한 gem갯수
          @iMyGemUID	           int,   --내가 가진 gem의 UID
          @iGemGoodsID             int, -- gem의 goodsid 
          @iMyGem                  int, -- 내가 가진  gem갯수 
          @bCash                   bit,
          @bShowWeb                bit,
          @iGoodsUID               int,
          @bSubSetGoods            bit,
...
IF NOT EXISTS(SELECT * FROM dbo.DurationItemObjectList(NOLOCK) WHERE Ownerlogin = @strBuyerLogin_ AND GoodsID = @iGemGoodsID)
BEGIN
    SELECT -12, 0
    RETURN
END
SELECT @iMyGem  = Duration, @iMyGemUID = DurationItemUID FROM dbo.DurationItemObjectList(NOLOCK) WHERE Ownerlogin = @strBuyerLogin_ AND GoodsID = @iGemGoodsID
...
--세트아닌 수량성 아이템중에서 이미 같은 아이템이 있는경우
    ...
    UPDATE dbo.DurationItemObjectList SET Duration = @iMyGem WHERE DurationItemUID = @iMyGemUID
    IF @@ERROR <> 0
        GOTO lbTran_Abort
...
lbTran_Success:
SET @strResultSQL = 'SELECT 0, ' +  CAST(@iMyGem AS nvarchar(10)) + ', ' + CAST(@iGemGoodsID AS nvarchar(10)) + ', ' + CAST(@iMyGemUID AS nvarchar(10)) + ', ' + CAST(@iGem AS nvarchar(10)) + ', ' + CAST(@iNumGoods AS nvarchar(10)) + @strGoodsUIDs + ', ' +  CAST(@iNumDurationSubSet AS nvarchar(10))  +  @strDurationUIDs
