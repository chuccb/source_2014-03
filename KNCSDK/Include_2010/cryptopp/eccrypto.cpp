...
template <class EC> OID DL_GroupParameters_EC<EC>::GetNextRecommendedParametersOID(const OID &oid)
{
	...
	const EcRecommendedParameters<EllipticCurve> *it = std::upper_bound(begin, end, oid, OIDLessThan<EllipticCurve>());
...
template <class EC> void DL_GroupParameters_EC<EC>::Initialize(const OID &oid)
{
	...
	const EcRecommendedParameters<EllipticCurve> *it = std::lower_bound(begin, end, oid, OIDLessThan<EllipticCurve>());
