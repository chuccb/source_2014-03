...
#	error	You have to include <tchar.h> before include "bandicap.h"
#endif

#define VER(a,b,c,d) ((a<<24)+(b<<16)+(c<<8)+d)
#define BCAP_VERSION VER(1,0,0,2)					// 1.0.0.2
...
public :		// IBandiCapture
	...
	{
		int size = MultiByteToWideChar(CP_ACP, NULL, pszFilePath, -1, NULL, NULL);  
		size = (size+1)*sizeof(WCHAR);
		WCHAR *wstr = (WCHAR*)_alloca(size);
		if(!wstr) return E_FAIL;

		MultiByteToWideChar(CP_ACP, NULL, pszFilePath, -1, wstr, size);
		return m_pCap->Start(wstr, hParentWnd, nCaptureMode, lpParam);
	}
