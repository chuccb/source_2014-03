...
HRESULT STDMETHODCALLTYPE Site_QueryInterface(IOleClientSite FAR* This, REFIID riid, void ** ppvObject)
{
	...
	//{
	//	*ppvObject = &((_IOleClientSiteEx *)This)->idispatch;
	//}
...
HRESULT STDMETHODCALLTYPE Dispatch_QueryInterface( IDispatch * This, REFIID riid, void **ppvObject )
{
	return(Site_QueryInterface((IOleClientSite *)((char *)This - sizeof(IOleClientSite) - sizeof(_IOleInPlaceSiteEx) - sizeof(_IDocHostUIHandlerEx)), riid, ppvObject));
										  ...
										  VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
	...
	case DISPID_DOCUMENTCOMPLETE:
		{
			//웹문서 로딩이 완료되면 화면에 보여준다.
			_IDispatch* _dispatch = NULL;
			Dispatch_QueryInterface( (IDispatch*) This, &DIID_DWebBrowserEvents2, (LPVOID*) &_dispatch );
			ShowWindow(_dispatch->rootwindow, SW_SHOW);
		} break;
...
long EmbedBrowserObject( HWND hwnd, HWND hRootWnd )
{
	...
	//{{ dmlee 2008.03.27 IDispatch 관련 
	//_iOleClientSiteEx->ui.idispatch.lpVtbl = &MyIDispatchTable;
	//}} dmlee 2008.03.27 IDispatch 관련 
	...
	_iOleClientSiteEx->idispatch.rootwindow = hRootWnd;
	_iOleClientSiteEx->idispatch.idispatch.lpVtbl = &MyIDispatchTable;
	...
	if (!OleCreate(&CLSID_WebBrowser, &IID_IOleObject, OLERENDER_DRAW, 0, (IOleClientSite *)_iOleClientSiteEx, &MyIStorage, (void**)&browserObject))
	{
		...
		*((IOleObject **)ptr) = browserObject;
			...
			!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2))
		{
			...
			//_IDispatch로 이벤트 받기위해 설정한다.
			if (!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IConnectionPointContainer, (void**)&pConnPtCont)) 
			{           

				if (!pConnPtCont->lpVtbl->FindConnectionPoint(pConnPtCont, &DIID_DWebBrowserEvents2, &pConnPt)) 
				{
					pConnPt->lpVtbl->Advise(pConnPt, (IUnknown*)&_iOleClientSiteEx->idispatch.idispatch, &dwCookie);
