...
Komfile::EKomfileError Komfile::Save( std::string filename,FILE_PROGRESS_CALLBACK progress)
{
	std::string tmpfilename = filename + ".tmp";
	std::ofstream file(tmpfilename.c_str(),std::ios_base::binary|std::ios_base::out);

	if(!file.is_open())
		return eKomfileError_Type_FileAccess;
	...
	while(i!=subfiles.end())
	{
		...
		{
#ifdef GATHER_PATCH_INFO
			if(progress( (writesize/totalsize), 0.0f ) == false)
#else GATHER_PATCH_INFO
...
#endif GATHER_PATCH_INFO
			{
				...
				return eKomfileError_Type_Success;
			}
		}		
	}
	file.close();
	...
	{
		DeleteFileForce(tmpfilename);
		return eKomfileError_Type_InvalidFile;
	}
