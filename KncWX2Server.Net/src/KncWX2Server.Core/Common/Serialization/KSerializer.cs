...
public sealed class KSerializer
{
    ...
    public bool PutMultiset<T>(
        IReadOnlyCollection<T> value,
        Func<KSerializer, T, bool> putElement)
    {
        return PutSequence(SerializeTag.Multiset, value, putElement);
    }

    public bool GetMultiset<T>(ICollection<T> value, ElementReader<T> readElement)
    {
        return GetSequence(SerializeTag.Multiset, value, readElement);
    }
