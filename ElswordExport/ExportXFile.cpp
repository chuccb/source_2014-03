    ...
    ) 
{
    ...
    switch(message) 
    {
        case WM_INITDIALOG:
			...
			CheckDlgButton(hWnd, IDC_CHECK_CHILDREN_NOT_ATTACHED, 
			...
			//
			{
				...
				SetDlgItemText( hWnd, IDC_EDIT_ANIM_FRAME_BEGIN, buff );
        ...
        case WM_COMMAND:
            ...
            case IDC_LOOPINGANIMATION:
				...
				// EDITED : 추가 옵션
				//
			case IDC_CHECK_CHILDREN_NOT_ATTACHED:
				pDialogOptions->m_bChildrenNotAttached = !pDialogOptions->m_bChildrenNotAttached;
				break;

			case IDC_CHECK_TEXTURE_FULL_PATH:
				pDialogOptions->m_bTextureFullPath = !pDialogOptions->m_bTextureFullPath;
				break;

			case IDC_EDIT_ANIM_FRAME_BEGIN:
				GetDlgItemText(hWnd,IDC_EDIT_ANIM_FRAME_BEGIN,buff,sizeof(buff) / sizeof(TCHAR));
			...
			case IDC_COMBO1:
				{
					...
					case CBN_SELCHANGE:
						int i = SendDlgItemMessage(hWnd, IDC_COMBO1,CB_GETCURSEL,0,0);
    ...
    )
{
	...
	// EDITED : 맥스 MODIFIER / PHYSIQUE 메뉴옵션을 받아와 업데이트 해준다.
	//
	Modifier* pPhyMod= NULL;
	IPhysiqueExport* pPhyExport= NULL;
    ...
    )
{
	...
	{
		matNodeTM = pNode->GetObjectTM(psc->m_iTime);	//offset이 적용된 tm
	}
	else
	{
		matNodeTM = pNode->GetNodeTM(psc->m_iTime);	//offset이 적용되지 않은 tm
	}
	//
...
e_Exit:
	...
	if( pPhyContextExport != NULL )
	{
		pPhyContextExport->ConvertToRigid(FALSE);
		pPhyContextExport->AllowBlending(FALSE);
	}
	//
    ...
    )
{
	...
	// EDITED : 06.07.27 테스트용 애니메이션 프레임구간 설정 부분
	//
	...
	for (iCurKey = 0, iCurTime = iStartTime; iCurTime <= iEndTime; iCurTime += iInterval, iCurKey++ )
	{
        for (iNode = 0; iNode < psc->m_cNodes; iNode++)
        {
			...
			// EDITED : 프레임시간을 당김
			//
			WRITE_DWORD(pbCur, iCurTime - g_AnimFrameAge.m_dwStartFrame * iInterval);
    ...
    if ( bAddEndKey )
    {
        ...
        for (iNode = 0; iNode < psc->m_cNodes; iNode++)
        {
			...
			// EDITED : 프레임시간을 당김
			//
			WRITE_DWORD(pbCur, iEndTime - g_AnimFrameAge.m_dwStartFrame * iInterval);
  ...
  //  }
	...
	// EDITED : 애니메이션 네임 셋팅부분
	// IDirectXFileData(psc->m_pAnimationSet)은 생성시 해당객체의 아이디(Ex.TID_D3DRMAnimationSet)를 주고 데이터등을 셋팅후 생성한다.
	//
    ...
    ) 
{
	...
	// EDITED : 파라메터 구조체를 통한 인터페이스 설정값을 불러오기 위해
	//
    ...
    g_DlgOptions.m_ExportDataOption			= EXPD_FRAME|EXPD_ANIMATION;	//	EDITED : 파라메타 변경.
	...
	_tcscpy( g_DlgOptions.m_strAnimName, pInterface->GetCurFileName() );//szFilename );

	//	EDITED : 추가 설정값.
	//
	g_DlgOptions.m_bLeftHandAxis = TRUE;
	g_DlgOptions.m_bHideBiped = TRUE;
    ...
    sc.m_ExportDataOption = g_DlgOptions.m_ExportDataOption;			//	EDITED : 파라메타 변경.
	...
	//	EDITED : 추가 설정값.
	//
	sc.m_bLeftHandAxis = g_DlgOptions.m_bLeftHandAxis;
	sc.m_bHideBiped = g_DlgOptions.m_bHideBiped;
    ...
    if ( g_DlgOptions.m_ExportDataOption&EXPD_ANIMATION )
    {
