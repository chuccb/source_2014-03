...
#define LUA_GET_USER_DEFINED_TYPE_VALUE( luaManager, key, dest, initValue ) \
{ \
	if( (luaManager).GetUserDefinedTypeValue( (key), (dest) ) == false ) \
		(dest) = (initValue); \
}
...
//}} robobeg : 2008-10-28
	...
	bool DoMemoryNotEncrypt( const char* pRealFile, long nSize )
    {
        ...
        //{{ 2009. 07. 17. jseop
        ASSERT( pRealFile != NULL && nSize > 0 );
        if( pRealFile  == NULL || nSize <= 0 )
        {
            return false;
        }
...
//}} robobeg : 2013-10-24
    ...
    // GET VALUE with user-defined type exported by lua-tineker *****

  //  template<typename TYPE>
  //  bool GetUserDefinedTypeValue( const wchar_t* pszName, TYPE& value, bool bLogOutput = true )
  //  {
		////KTDXPROFILE();
		//char charBuf[511] = {0,};
		//WideCharToMultiByte( CP_UTF8, 0, pszName, -1, charBuf, 511, NULL, NULL );
		//return GetUserDefinedValue( charBuf, value, bLogOutput );
  //  }

    template<typename TYPE>
    bool GetUserDefinedTypeValue( const char* pszName, TYPE& value, bool bLogOutput = true );
...
#ifdef  _USE_LUAJIT_
    enum EJitCompileMode
    {
        JIT_COMPILE_MODE_OFF = 0,
        JIT_COMPILE_MODE_ON = 1,
        JIT_COMPILE_MODE_FLUSH = 2,
    };
    void    SetJitCompileMode( EJitCompileMode eMode );
#endif  _USE_LUAJIT_
...
#ifdef  _USE_LUAJIT_
    void    _SetJitCompileModeRecursively( lua_State* L, EJitCompileMode eMode );
#endif  _USE_LUAJIT_
...
template<typename TYPE>
bool KLuaManager::GetUserDefinedTypeValue( const char* pszName, TYPE& value, bool bLogOutput )
{
    if ( Get(pszName) == true )
    {
        if ( lua_isuserdata(m_pkLuaState, -1) != 0 )
        {
            value = lua_tinker::pop<TYPE>( m_pkLuaState );
            return true;
        }
        else
        {
