...
//#pragma comment( lib, "../lib/SystemAnalysis" )
...
public:
    ...
    std::string GetSystemCpuData();
    std::string GetSystemDirectXVersion();
    std::string GetSystemDriverVersion();
    std::string GetSystemFreeHardCapacity();
    std::string GetSystemGraphicCardData();
    std::string GetSystemOsData();
    std::string GetSystemRamData();
    std::string GetSystemSoundData();
    
public:
    //사운드정보는 callback함수로 부르기때문에 전역 변수가 필요하다.
    std::string m_strSystemSoundData;
};
