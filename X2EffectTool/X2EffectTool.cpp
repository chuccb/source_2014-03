...
CBandiCaptureLibrary	g_bandiCaptureLibrary;	//캡쳐 라이브러리.
...
INT WINAPI WinMain( HINSTANCE hInstance, HINSTANCE, LPSTR, int )
{
    ...
    //FILE*                   m_fpStream;
	//FILE*                   m_fpStreamError;

	//AllocConsole();
	//m_fpStream		= freopen("CONOUT$", "a", stdout); // redirect printf to console
	//m_fpStreamError = freopen("CONOUT$", "a", stderr); // redirect printf to console
	...
	DXUTCreateWindow( L"X2EffectTool", NULL, hIcon );
	DXUTCreateDevice( D3DADAPTER_DEFAULT, true, 1280, 960, IsDeviceAcceptable, ModifyDeviceSettings );
	//DXUTCreateDevice( D3DADAPTER_DEFAULT, true, 1024, 768, IsDeviceAcceptable, ModifyDeviceSettings );
	//DXUTCreateDevice( D3DADAPTER_DEFAULT, true, 1280, 1024, IsDeviceAcceptable, ModifyDeviceSettings );
...
void ToggleVideoCapture()
{
	...
	memset( wszPath, 0, sizeof(WCHAR)*(MAX_PATH+1));
	BROWSEINFO bInfo;
	ZeroMemory( &bInfo, sizeof(BROWSEINFO));
	bInfo.hwndOwner = g_pKTDXApp->GetHWND();
	bInfo.pszDisplayName = wszPath;
	bInfo.lpszTitle = L"Capture Directory";
	bInfo.ulFlags = BIF_BROWSEFORCOMPUTER | BIF_USENEWUI ;
	...
	// 캡처를 시작합니다.	
	if(g_bandiCaptureLibrary.IsCapturing()==FALSE)
	{
		ITEMIDLIST  *pidlBrowse;
		pidlBrowse = ::SHBrowseForFolder(&bInfo);
		if( pidlBrowse != NULL)
		{
			if( ::SHGetPathFromIDList(pidlBrowse, wszPath) == false)
			{
				MessageBox(NULL, L"Error!", L"캡쳐실패!", MB_OK);
				return;
			}
		}
		else
		...
		if(g_bandiCaptureLibrary.IsCreated()==FALSE)
		{
			// 실패시 DLL과 .H파일의 버전이 동일한지 여부등을 
			// 확인해 보시기 바랍니다.
			if(FAILED(g_bandiCaptureLibrary.Create(BANDICAP_RELEASE_DLL_FILE_NAME)))
				ASSERT(0);
		...
		if(g_bandiCaptureLibrary.IsCreated())
		{
			...
			g_bandiCaptureLibrary.MakePathnameByDateW(wszPath, L"Elsword", 
				pathName, MAX_PATH);		
			...
			BCapConfigPreset(&cfg, BCAP_PRESET_HALFSIZE);	//화면싸이즈의 반으로 캡쳐.

			g_bandiCaptureLibrary.CheckConfig(&cfg);	// 잘못된 설정 바로 잡기
			g_bandiCaptureLibrary.SetConfig(&cfg);		// 설정 적용
			g_bandiCaptureLibrary.SetMinMaxFPS(30, 60);	// 최소, 최대 프레임 설정
		


			// 캡처를 시작합니다.			
			HRESULT hr = g_bandiCaptureLibrary.Start(pathName, NULL, 
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
	...
	CX2EffectToolMain* pStage = new CX2EffectToolMain;
	//CX2DialogToolMain* pStage = new CX2DialogToolMain;
	//CFileRenameTool* pStage = new CFileRenameTool;
	//CX2TextureViewer* pStage = new CX2TextureViewer;
	//CX2NPCTool* pStage = new CX2NPCTool;
	//CX2SkillTreeTool* pStage = new CX2SkillTreeTool;
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	if( SUCCEEDED( pd3dDevice->BeginScene() ) )
	{
		...
		//{{081001.hoons.
		if( g_bandiCaptureLibrary.IsCapturing() == TRUE )
		{
			g_bandiCaptureLibrary.Work( (LONG_PTR)pd3dDevice );
		}
		//}}
	}
}
...
void RenderText()
{
	...
	//{{081001.hoons.캡쳐진행 상태표시
	WCHAR buff[256] = {0};
	if( g_bandiCaptureLibrary.IsCreated() == TRUE )
	{
		wsprintf( buff, L"Is Capturing.? %d \n", static_cast<int>(g_bandiCaptureLibrary.IsCapturing()) );
	}

	txtHelper.SetInsertionPos( 5, 30 );
	txtHelper.SetForegroundColor( D3DXCOLOR( 1.0f, 1.0f, 0.0f, 1.0f ) );
...
LRESULT CALLBACK MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, bool* pbNoFurtherProcessing, void* pUserContext )
{
	...
	*pbNoFurtherProcessing = g_DialogResourceManager.MsgProc( hWnd, uMsg, wParam, lParam );
	...
	//{{081001.hoons. start and End of capture
